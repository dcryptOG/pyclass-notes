...
# WHEN A CLASS IS DEFINED... only the description for the object is defined, NO MEMORY or storage is ALLOCATED (), like def function must be executed to have an effect.

# NOTE When a CLASS DEFINITION is entered... a NEW NAMESPACE is created, and used as the LOCAL SCOPE — thus, ALL ASSIGNMENTS to local variables go into this new namespace. Function definitions BIND the NEW function NAME here.

# The ORIGINAL LOCAL SCOPE == scope in effect just BEFORE the CLASS DEFINITION was entered) is reinstated, and the CLASS OBJECT is bound here to the CLASS NAME given in the CLASS DEFINITION HEADER.
...
#Class obj used to create new object instances (instantiation) of that class. The procedure to create an object is similar to a function call.
...
# This is because, whenever an object calls its method, the INSTANCE OBJECT itself is passed as the FIRST ARG. 

#!ob.func() === MyClass.func(ob).

# In general, calling a method with a list of n arguments is equivalent to  calling the corresponding function with an ARG LIST that is created by INSERTING the METHOD'S OBJ BEFORE the 1ST ARG argument.For these reasons, the 1st ARG of the function in class must be the OBJ itself. This is conventionally called SELF. 
...
#If the name denotes Class attribute that is a FUNCTION OBJ, a METHOD OBJ is created by PACKING (pointers to) the INSTANCE OBJ and the function object just found together in an ABSTRACT OBJ: this is the method object. 
...
#?CLASS OBJ ATTRIBUTE or CLASS VAR
    species = "bird"
    #Shared by all instances, USE IMMUTABLE DATA TYPE
...
#* Multiple Inheritance = class can be derived from more than one base classes in Python.
...
#* Multilevel inheritance =inherit form a derived class. (be of any depth in Python.)
...
# MOST PURPOSES, in the SIMPLEST CASE, you can think of the search for attributes inherited from a parent class as 
# DEPTH-FIRST, 
# left-to-right, 
...
# MORE COMPLES than that; the method resolution order CHANGES DYNAMICALLY to support cooperative calls to super(). This approach is known in some other multiple-inheritance languages as CALL-NEXT-METHOD is more powerful than the super call found in single-inheritance languages.
