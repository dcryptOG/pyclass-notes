...
# NOTE USUALLY the statements inside a CLASS DEFINITION are FUNCTION DEFINTIONS, but OTHER statements are allowed.

# When a CLASS DEFINITION is entered, a NEW NAMESPACE is created, and used as the LOCAL SCOPE — thus, all assignments to local variables go into this new namespace. In particular, function definitions BIND the new function NAME here.
...
#
# NOTE EX (assuming the above class):
...
# since MyClass.f is a FUNCTION, but x.i is not
# , since MyClass.i is not.
# x.f is not the same thing as MyClass.f — it is a METHOD OBJECT
